...
serve(async (req) => {
  ...
  try {
    ...
    const { nombre, apellidos, email, capacitaciones } = record;
    ...
    const caps = (capacitaciones || '').split(' | ').filter(Boolean);

    const MAILGUN_API_KEY = Deno.env.get('MAILGUN_API_KEY')!;
    const MAILGUN_DOMAIN = 'mg.asobares.org';
    const FROM = 'Asobares Armenia <ceo@vamosarayar.com>';

    const capsRows = caps.map(c => `
      <tr>
        <td style="padding:10px 0;border-bottom:1px solid #e8f5e9;">
          <span style="display:inline-block;width:8px;height:8px;background:#2db84b;border-radius:50%;margin-right:10px;vertical-align:middle;flex-shrink:0;"></span>
          <span style="font-size:14px;color:#1a2e1a;">${c}</span>
        </td>
      </tr>`).join('');
  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#f5f5f5;padding:32px 0;">
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 2px 16px rgba(0,0,0,0.08);">
        ...
        <tr>
          <td style="padding:0;">
            <img src="https://asobares.org/wp-content/uploads/2026/08/Banner-forms-5-scaled.jpg" alt="Inscripción a Formación o Capacitación" style="width:100%;display:block;" />
          </td>
        </tr>
        ...
        <tr>
          <td style="padding:24px 40px 32px;">
            <p style="margin:0 0 24px;font-size:15px;color:#475569;line-height:1.7;text-align:center;">
              Quedaste inscrito(a) en el programa de Fortalecimiento Empresarial para el sector de bares y restaurantes del Quindío.
            </p>
            ...
            <table width="100%" cellpadding="0" cellspacing="0" style="background:#f8fff9;border:1px solid #c8e6c9;border-radius:12px;margin:0 0 24px;">
              <tr>
                <td style="padding:20px 24px;">
                  <p style="margin:0 0 14px;font-size:11px;font-weight:700;color:#4a7c59;letter-spacing:2px;">TUS CAPACITACIONES SELECCIONADAS</p>
    ...
    body.append('subject', '¡Inscripción confirmada! Capacitación Fortalecimiento Empresarial — Armenia');
